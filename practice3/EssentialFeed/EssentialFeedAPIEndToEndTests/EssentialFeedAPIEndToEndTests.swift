...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_matchesFixedTestData() {
        ...
        case let .success(feed):
            XCTAssertEqual(feed.count, 8, "Expected 8 images in the test account feed, got \(feed.count) instead")
    ...
    private func getFeedResult(file: StaticString = #filePath, line: UInt = #line) -> FeedLoader.Result? {
        ...
        var receievedResut: FeedLoader.Result?
        loader.load { result in
            receievedResut = result
            exp.fulfill()
        }
        
        wait(for: [exp], timeout: 5.0)
        
        return receievedResut
    }
