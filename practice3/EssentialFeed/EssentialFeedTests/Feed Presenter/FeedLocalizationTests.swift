...
//
//  Created by ChenZhen on 21/1/25.
//
...
final class FeedLocalizationTests: XCTestCase {
    ...
    private func allLocalizationBundles(in bundle: Bundle, file: StaticString = #file, line: UInt = #line) -> [LocalizedBundle] {
        return bundle.localizations.compactMap { localization in
            guard let path = bundle.path(forResource: localization, ofType: "lproj"), let localizedBundle = Bundle(path: path) else {
                XCTFail("Localization bundle not found for \(localization) in \(bundle.bundlePath)", file: file, line: line)
                return nil
            }
    ...
    private func allLocalizedStringKeys(
        ...
        let keys = bundles.compactMap { current -> [String]? in
            guard
